/*
 * index.js — Avalon online server (Phase 1).
 *
 ...
 *   client -> server: create | join | reconnect | setName | updateSettings | startDeal | requestPrivate | leave
 *   server -> client: created | joined | room_state | private_state | error | pong
 *
 * SECURITY INVARIANT: private_state is sent with ws.send to a single socket only.
 * publicState() (broadcast) never contains any role. See rooms.js.
 */
...
setInterval(() => manager.sweep(), 1000 * 60 * 10).unref?.();
