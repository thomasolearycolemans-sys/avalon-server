/*
 * rooms.js — in-memory room + session management for Avalon online (Phase 1).
 *
 * Responsibilities:
 ...
 *   - hold the authoritative deal server-side and expose per-player private state
 *   - room lifecycle: lobby -> dealing -> revealed  (Phase 1 stops at dealing)
 *
 * Nothing here talks to a socket directly; index.js wires messages to these methods.
 * That keeps the game/authority logic testable without a network (see test-harness.js).
 */
...
class Room {
  constructor(code) {
    ...
    // onAdvance is set by index.js so the timer can push state; kept out of state payloads.
  ...
  markDisconnected(socketId) {
    ...
    if (p) { p.connected = false; p.socketId = null; this.touch(); }
    return p;
  }
  ...
  publicGame() { return this.game ? Avalon.publicGameView(this.game) : null; }
...
class RoomManager {
  ...
  sweep() {
    for (const [code, room] of this.rooms) if (room.expired()) this.rooms.delete(code);
  }
}
